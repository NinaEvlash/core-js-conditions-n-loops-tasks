...
function isPositive(number) {
  let res = true;
  if (number === 0 || number % 2 === 0) res = true;
  else res = false;
  return res;
}
...
function canQueenCaptureKing(queen, king) {
  ...
  else if (queen.x === queen.y && king.x === king.y) res = true;
  else if (queen.x + queen.y === king.x + king.y) res = true;
  else res = false;
  return res;
}
...
function isIsoscelesTriangle(a, b, c) {
  let res = false;
  if (a === b && !(a - b >= c || a + b <= c)) res = true;
  else if (a === c && !(a - c >= b || a + c <= b)) res = true;
  else if (b === c && !(b - c >= a || b + c <= a)) res = true;
  return res;
}
...
function sortByAsc(arr) {
  ...
  for (let i = 0; i < arrSort.length - 1; i += 1) {
    let indexMin = i;
    for (let j = i + 1; j < arrSort.length; j += 1) {
      if (arrSort[indexMin] > arrSort[j]) {
        indexMin = j;
      }
    }
    if (indexMin !== i) {
      [arrSort[i], arrSort[indexMin]] = [arrSort[indexMin], arrSort[i]];
    }
  }
  return arrSort;
}
